  ...
  return (
    <div style={styles.container}>
      ...
      {displayRides.length === 0 ? (
        ...
        if (userLocation && ride.from?.coordinates) {
            let rideLat, rideLng;
            if (Array.isArray(ride.from.coordinates)) {
                 // Assume GeoJSON [lng, lat]
                 rideLng = ride.from.coordinates[0];
                 rideLat = ride.from.coordinates[1];
            } else if (typeof ride.from.coordinates === 'object') {
                 rideLat = ride.from.coordinates.lat;
                 rideLng = ride.from.coordinates.lng;
            }
            
            if (rideLat && rideLng) {
                const dist = calculateDistance(userLocation.latitude, userLocation.longitude, rideLat, rideLng);
