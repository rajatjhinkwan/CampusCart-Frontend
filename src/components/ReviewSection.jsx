...
const ReviewSection = ({ targetId, targetType }) => {
  ...
  const handleAiHelp = () => {
    const templates = [
      "The service was outstanding and exceeded my expectations!",
      "I had a wonderful experience. The quality is top-notch.",
      "Good value for money. I would recommend this to others.",
      "The process was smooth and the results were great.",
    ...
    if (text.trim().length > 5) {
      ...
      const enhancements = [
        `I must say, ${text} It was a delightful experience.`,
        `${text} - highly recommended!`,
        `To be honest, ${text} - 5 stars from me!`,
        `I really appreciated that ${text}`
      ];
      setText(enhancements[Math.floor(Math.random() * enhancements.length)]);
      toast.success("AI enhanced your review!");
    } else {
      // Generate new text
      setText(templates[Math.floor(Math.random() * templates.length)]);
      toast.success("AI generated a review idea!");
    }
  };
