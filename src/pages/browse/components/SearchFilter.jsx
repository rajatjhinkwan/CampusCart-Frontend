...
const SearchFilter = ({ filters = {}, setFilters, type = "Products" }) => {
  ...
  const vehicleTypes = ["Car", "Bus", "Taxi", "Bike", "Auto", "Truck"];
  const frequencyOptions = ["Daily", "One-time", "Weekly"];
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const subcategories = cats.filter((c) => c.parent);
        const options = subcategories.length ? subcategories : cats;
        if (mounted) setProductCategories(options);
      } catch {
        // fallback: empty list
  ...
  useEffect(() => {
    ...
    // Transport-specific
    setVehicleType(filters.type || "");
    setFrequency(filters.frequency || "");
  }, [filters]);
  ...
  const handleApplyFilters = () => {
    ...
    if (type === "Products") {
      ...
      if (vehicleType) applied.type = vehicleType;
      if (frequency) applied.frequency = frequency;
      if (location) applied.fromCity = location;
      if (minPrice !== "" && minPrice !== null) applied.minPrice = minPrice;
      if (maxPrice !== "" && maxPrice !== null) applied.maxPrice = maxPrice;
    } else {
      if (category) applied.category = category;
