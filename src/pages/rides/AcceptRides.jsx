...
const AcceptRides = () => {
  ...
  */
  ...
  useEffect(() => {
    const compute = async () => {
      if (!selectedRide || !selectedRide.from || !selectedRide.to) return;
      const a = getCoords(selectedRide.from);
      const b = getCoords(selectedRide.to);
      if (!a || !b) return;
      
      // --- OSRM ROUTING REPLACEMENT ---
      try {
        const osrmBase = import.meta.env.VITE_OSRM_SERVER_URL || DEFAULT_OSRM || 'https://router.project-osrm.org/route/v1';
        // Check if we are using the proxy path or full URL
        const finalBase = osrmBase.startsWith('http') ? osrmBase : `${API_BASE}${osrmBase}`;
        
        // Use 'driving' profile
        const url = `${finalBase}/driving/${a.lng},${a.lat};${b.lng},${b.lat}?overview=full&geometries=geojson`;
        ...
        if (res.ok) {
            ...
            if (data.routes && data.routes.length > 0) {
                ...
                // If we need the route geometry (LiveMap might use it if passed)
                if (route.geometry && route.geometry.coordinates) {
                    // OSRM returns [lon, lat], Leaflet needs [lat, lon]
                    const latLngs = route.geometry.coordinates.map(c => [c[1], c[0]]);
                    setDetailsRoute(latLngs);
                }
                return;
            }
        }
      ...
      try {
        ...
        // ... Google implementation commented out
      } catch (e) { ... }
      */
      ...
      setDetailsDistanceKm(d);
      setDetailsEtaMins(Math.round(d * 2)); // Rough estimate: 30km/h average speed in city = 2 mins per km
    };
    compute();
  }, [selectedRide]);
                ...
                <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
                  <div style={styles.modalBody}>
                    ...
                    <div style={{ marginTop: 12, height: 240, borderRadius: 8, overflow: 'hidden', border: '1px solid #e5e7eb' }}>
                      <LiveMap
                        from={getCoords(selectedRide?.from)}
                        to={getCoords(selectedRide?.to)}
