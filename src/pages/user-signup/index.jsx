...
export default function Signup() {
    ...
    */
    ...
    const handleLocationSearch = async (query) => {
        ...
        // --- GOOGLE PLACES AUTOCOMPLETE (COMMENTED) ---
        /*
        ...
        */
        ...
        // --- BACKUP: PHOTON (ENABLED) ---
        try {
            const API_BASE = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000';
            // Use backend proxy to avoid CORS/mixed-content issues if available, or fall back to direct if needed (though backend is safer)
            // If VITE_PHOTON_PROXY_BASE is defined, use that.
            const photonBase = import.meta.env.VITE_PHOTON_PROXY_BASE || `${API_BASE}/api/geo/photon`;
            
            // NOTE: If using direct Photon API from client (not recommended due to mixed content on https), it would be https://photon.komoot.io/api/
    ...
    const handleSubmit = async (e) => {
        ...
        if (result.success) {
            ...
            if (result.devOtp) {
                ...
                toast.success(`OTP auto-filled: ${result.devOtp} (Development Mode)`);
            }
    ...
    const handleResendOtp = async () => {
        setError("");
        const result = await resendOtp({ userId: userIdForOtp, email: formData.email });
        if (result.success) {
            if (result.devOtp) {
                setOtp(result.devOtp);
                toast.success(`New code: ${result.devOtp} (Development Mode)`);
            }
        } else {
                ...
                <div style={modalStyles.overlay}>
                    <div style={modalStyles.content}>
                        ...
                        <form onSubmit={handleVerifyOtp}>
                            ...
                            <button type="button" onClick={handleResendOtp} style={{...styles.button, marginTop: '12px', background: '#fff', color: '#2563eb', border: '1px solid #2563eb'}}>
                                Resend Code
                            </button>
                        </form>
                    </div>
