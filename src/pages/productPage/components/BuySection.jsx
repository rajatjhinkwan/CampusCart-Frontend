...
const BuySection = ({ product }) => {
  ...
  const handleAction = async () => {
    ...
    // Identify seller ID
    const sellerId = product.seller?._id || product.seller?.id || product.seller || product.user?._id || product.user?.id || product.user;
    if (!sellerId) {
      toast.error("Seller information missing");
      return;
    }

    const myId = useUserStore.getState().user?._id || useUserStore.getState().user?.id;
    if (String(myId) === String(sellerId)) {
