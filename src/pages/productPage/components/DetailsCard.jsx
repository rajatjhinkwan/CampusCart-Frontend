...
import { formatCategory, formatLocation } from "../../../utils/formatDisplay";
...
const DetailsCard = ({ data }) => {
  ...
  return (
    <div style={styles.card}>
      <h2 style={styles.title}>{data.title || "Untitled"}</h2>
      <span style={styles.sub}>{categoryLabel}</span>

      <div style={styles.section}>
        <p style={styles.label}>Location</p>
        <p style={styles.info}>{locationLabel}</p>
      </div>
      ...
      {(data.brand || data.model || data.yearOfPurchase) && (
        <div style={styles.section}>
          <p style={styles.label}>Specifications</p>
          <p style={styles.info}>
            {[data.brand, data.model, data.yearOfPurchase].filter(Boolean).join(" • ")}
          </p>
        </div>
      )}

      {(data.imeiLast4 || data.vehicleNumberLast4) && (
        <div style={styles.section}>
          <p style={styles.label}>Verification</p>
          <p style={styles.info}>
            {data.imeiLast4 ? `IMEI: ************${data.imeiLast4}` : ""}
            {data.imeiLast4 && data.vehicleNumberLast4 ? " • " : ""}
            {data.vehicleNumberLast4 ? `Vehicle: ******${data.vehicleNumberLast4}` : ""}
          </p>
        </div>
      )}
