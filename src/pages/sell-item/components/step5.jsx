...
export default function Step5({ form = {}, onNext, onBack, categoryType }) {
  ...
  useEffect(() => {
    setFormData((prev) => ({
      ...
      mapLink: form.mapLink ?? prev.mapLink,
      lat: form.lat ?? prev.lat,
      lng: form.lng ?? prev.lng,
    }));
  }, [form.area, form.city, form.state, form.pincode, form.address, form.mapLink, form.lat, form.lng]);
  ...
  const requiresPincode = categoryType === "Room" || categoryType === "Job";
  const isFormValid = formData.city.trim() && formData.state.trim() &&
    (requiresPincode ? String(formData.pincode || "").trim() : true) &&
