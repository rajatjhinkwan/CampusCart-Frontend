...
export default function Step6ReviewPublish({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => navigate("/user-listings"), 2000);
    return () => clearTimeout(timer);
  }, [isSubmitted, navigate]);

  const isRental = categoryType === "Product" && priceDetails.transactionType === "rent";
  ...
  return (
    <>{!isSubmitted ? (
        ...
        <div style={styles.sectionsContainer}>
          ...
          <div style={styles.sectionCard}>
            ...
            <ul style={styles.list}>
              ...
              {categoryType === "Product" && (
                <li style={styles.listItem}>
                  <b style={styles.listLabel}>{isRental ? "Rental Price:" : "Price:"}</b>{" "}
                  <span>
                    {priceDetails.price
                      ? `₹ ${Number(priceDetails.price).toLocaleString("en-IN")}${isRental ? ` / ${priceDetails.rentalPeriod || "Monthly"}` : ` (${priceDetails.mode || "Fixed"})`}`
                      : "N/A"}
                  </span>
                </li>
              )}
              {categoryType === "Product" && isRental && priceDetails.securityDeposit && (
                <li style={styles.listItem}>
                  <b style={styles.listLabel}>Security Deposit:</b>{" "}
                  <span>₹ {Number(priceDetails.securityDeposit).toLocaleString("en-IN")}</span>
                </li>
              )}
              {categoryType === "Transport" && (
                <li style={styles.listItem}>
                  <b style={styles.listLabel}>Price per seat:</b>{" "}
                  <span>{priceDetails.price ? `₹ ${Number(priceDetails.price).toLocaleString("en-IN")}` : "Free / negotiable"}</span>
                </li>
              )}
              {categoryType === "Room" && (
                <li style={styles.listItem}>
            ...
            <div style={styles.sectionCard}>
              ...
              <ul style={styles.list}>
                <li style={styles.listItem}>
                  ...
                  <span>{formData.serviceType || formData.category || "N/A"}</span>
                </li>
              </ul>
            </div>
          )}
          ...
          {categoryType === "Transport" && (
            <div style={styles.sectionCard}>
              ...
              <ul style={styles.list}>
                <li style={styles.listItem}>
                  <b style={styles.listLabel}>Vehicle:</b>{" "}
                  <span>{formData.vehicleName || formData.vehicleType || "N/A"}</span>
                </li>
                <li style={styles.listItem}>
                  <b style={styles.listLabel}>Route:</b>{" "}
                  <span>{location.city || "From"} → {formData.toCity || "To"}</span>
                </li>
                <li style={styles.listItem}>
                  <b style={styles.listLabel}>Departure:</b>{" "}
                  <span>{formData.departureTime || "N/A"} ({formData.frequency || "One-time"})</span>
                </li>
                <li style={styles.listItem}>
                  <b style={styles.listLabel}>Seats:</b>{" "}
                  <span>{formData.seatsAvailable ?? "N/A"}</span>
                </li>
              </ul>
            </div>
          )}
      ...
      <div style={styles.container}>
        ...
        <p style={styles.subheading}>Your listing is now live. Redirecting to your listings…</p>
      </div>
    )}</>
  );
}
