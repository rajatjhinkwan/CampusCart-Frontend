...
import { resolveMediaUrl } from "../../utils/formatDisplay";
...
const SOCKET_API = import.meta.env.DEV
  ? (import.meta.env.VITE_API_DEV_URL || window.location.origin)
...
export default function MessagesPage() {
    ...
    const activeConversation = conversations.find(
        (c) => String(c._id) === String(selectedConversation)
    );
    ...
    useEffect(() => {
        if (isAuthenticated && accessToken && !socketInitRef.current) {
            ...
            const newSocket = io(`${SOCKET_API}/chat`, {
    ...
    useEffect(() => {
        ...
        if (convId && conversations.length > 0) {
            ...
            if (match) selectConversation(match._id);
            return;
        }
        if (!selectedConversation && conversations.length > 0 && !isMobile) {
            selectConversation(conversations[0]._id);
        }
    }, [searchParams, conversations, selectConversation, selectedConversation, isMobile]);
    ...
    const handleSaveEdit = async (msgId) => {
        ...
        try {
            await axios.put(`/api/messages/${msgId}`, { content: editContent });
            setEditingMessageId(null);
            setEditContent("");
            const store = useUserStore.getState();
            if (store.selectedConversation) {
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ width: '3rem', height: '3rem', borderRadius: '0.5rem', overflow: 'hidden', backgroundColor: '#e2e8f0', flexShrink: 0 }}>
                        {item.image ? (
                            <img src={resolveMediaUrl(item.image, SOCKET_API) || item.image} alt={item.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        ) : (
                ...
                <div style={{
                    ...
                    <div style={styles.conversationList}>
                        {conversationsLoading && conversations.length === 0 && (
                            <div style={{ padding: '1rem', textAlign: 'center', color: '#6b7280', fontSize: '0.875rem' }}>Loading conversations...</div>
                        )}
                        {!conversationsLoading && conversations.length === 0 && (
                        ...
                        {sortedConversations.map((conversation) => {
                            ...
                            const isActive = String(selectedConversation) === String(conversation._id);
                                                    ...
                                                    {Array.isArray(msg.attachments) && msg.attachments.length > 0 && (
                                                        <div style={{ marginTop: '0.5rem', display: 'grid', gridTemplateColumns: msg.attachments.length === 1 ? '1fr' : 'repeat(auto-fit, minmax(80px, 1fr))', gap: '0.25rem', maxWidth: '300px' }}>
                                                            {msg.attachments.map((url, i) => {
                                                                ...
                                                                const fullSrc = resolveMediaUrl(src, SOCKET_API) || src;
                                                                return (
                                                                    <div key={i}
